...
class QueryRewriteAgent:
    ...
    def batch_process_queries(self, queries: List[str], strategy: str = "rewrite") -> List[Dict[str, str]]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all tasks
            future_to_index = {
                executor.submit(self._process_query_with_retry, query, strategy): i for i, query in enumerate(queries)
            }
    ...
    def batch_process_multiple_strategies(
        self, queries: List[str], strategies: List[str] = None
