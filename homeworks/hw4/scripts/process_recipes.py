...
def process_recipe(row: Dict[str, str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        full_text_parts = [name, description, " ".join(ingredients), " ".join(steps), " ".join(tags)]
...
def select_longest_recipes(recipes: List[Dict[str, Any]], top_n: int = 200) -> List[Dict[str, Any]]:
    ...
    if longest_recipes:
        ...
        avg_minutes = sum(r.get("minutes", 0) for r in longest_recipes if r.get("minutes", 0) > 0) / len(
            [r for r in longest_recipes if r.get("minutes", 0) > 0]
        )
...
def main():
    ...
    print(
        f"Average ingredients per recipe: {sum(r['n_ingredients'] for r in longest_recipes) / len(longest_recipes):.1f}"
    )
    print(f"Average steps per recipe: {sum(r['n_steps'] for r in longest_recipes) / len(longest_recipes):.1f}")
    print(
        f"Average cooking time: {sum(r['minutes'] for r in longest_recipes if r['minutes'] > 0) / len([r for r in longest_recipes if r['minutes'] > 0]):.1f} minutes"
    )
