...
"""
    ...
    def generate_queries_parallel(
        self, recipes: List[Dict[str, Any]], max_queries: int = 100, max_workers: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            future_to_recipe = {
                executor.submit(self.process_single_recipe, recipe): recipe for recipe in target_recipes
            }

            # Collect results with progress bar
            for future in tqdm(as_completed(future_to_recipe), total=len(future_to_recipe), desc="Processing recipes"):
...
def filter_complex_recipes(
    recipes: List[Dict[str, Any]], min_steps: int = 5, min_ingredients: int = 4
) -> List[Dict[str, Any]]:
    ...
    for recipe in recipes:
        ...
        _ingredients_text = " ".join(recipe.get("ingredients", [])).lower()

        # Prioritize recipes with specific techniques/equipment
        has_specific_techniques = any(
            term in steps_text
            for term in [
                "temperature",
                "degrees",
        ...
        if (
            has_enough_steps
            and has_enough_ingredients
            and (has_description or has_cooking_time or has_specific_techniques)
        ):
