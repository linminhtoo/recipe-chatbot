...
class RetrievalEvaluator(BaseRetrievalEvaluator):
    ...
    def print_final_summary(self, results: List[Dict[str, Any]]):
        ...
        print(
            f"   • Target found (any rank): {final_metrics['queries_found']} ({final_metrics['queries_found'] / final_metrics['total_queries'] * 100:.1f}%)"
        )
        print(
            f"   • Target not found:        {final_metrics['queries_not_found']} ({final_metrics['queries_not_found'] / final_metrics['total_queries'] * 100:.1f}%)"
        )
