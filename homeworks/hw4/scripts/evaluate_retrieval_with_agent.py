...
from evaluation_utils import BaseRetrievalEvaluator, load_queries, compare_retrieval_systems, print_comparison_results
...
class AgentRetrievalEvaluator(BaseRetrievalEvaluator):
    ...
    def evaluate_all_queries_with_strategy(
        self, queries: List[Dict[str, Any]], strategy: str, top_k: int = 5
    ) -> List[Dict[str, Any]]:
...
def main():
    ...
    # Find best strategy
    best_strategy = max(strategy_results.keys(), key=lambda s: strategy_results[s]["metrics"]["recall_at_5"])
    ...
    enhanced_evaluator.save_results(
        enhanced_results, results_enhanced_path, experiment_name=f"enhanced_{best_strategy}"
    )
