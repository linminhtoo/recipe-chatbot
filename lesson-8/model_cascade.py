...
def simulate_cascade(test_df: pd.DataFrame, thresholds: dict) -> None:
    ...
    _total_both_models_cost = total_proxy_only_cost + total_oracle_only_cost
    ...
    cascade_accuracy = sum(cp == op for cp, op in zip(cascade_predictions, test_df["oracle_prediction"])) / len(
        cascade_predictions
    )
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=32) as executor:
        ...
        train_proxy_futures = [executor.submit(process_doc, "gpt-4o-mini", text) for text in train_df["text"].tolist()]

        # Run train oracle predictions
        print("Running train oracle (gpt-4o) predictions...")
        train_oracle_futures = [executor.submit(process_doc, "gpt-4o", text) for text in train_df["text"].tolist()]

        # Run test proxy predictions
        print("Running test proxy (gpt-4o-mini) predictions...")
        test_proxy_futures = [executor.submit(process_doc, "gpt-4o-mini", text) for text in test_df["text"].tolist()]

        # Run test oracle predictions
        print("Running test oracle (gpt-4o) predictions...")
        test_oracle_futures = [executor.submit(process_doc, "gpt-4o", text) for text in test_df["text"].tolist()]
    ...
    _cascade_results = simulate_cascade(test_df, train_thresholds)
